...
overlap_null_distrib <- function(x, y, rand_both, x_convert = FALSE, y_convert = FALSE, x_cat = FALSE, y_cat = FALSE,
                                 threshold = 0.2, n_iterations = 10, x_model = 'random', y_model = 'random', progress = TRUE) {
  # progress bar
  ...
  repeat {
    ...
    if (rand_both == TRUE) {
      repeat {
        if (y_model == 'random') {
          y_sim <- random_raster_sim(x)
        } else if (y_model == 'gaussian') {
          y_sim <- gauss_random_field_sim(x)
        } else if (y_model == 'random_cluster') {
          y_sim <- mod_random_clust_sim(x)
        }
